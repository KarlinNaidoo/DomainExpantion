...
- A planet or 3D world, if added, is a read-only view. It must not start agents or live on the `delegate` path. `domain-expantion planet` serves that view from the registry.
